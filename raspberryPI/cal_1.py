    ...
    if ret:
        ...
        try:
            ...
            for box, track_id in zip(boxes, track_ids):
                ...
                if track_id not in track_history:
                    track_history[track_id] = [time.strftime('%Y-%m-%d-%H-%M-%S', p_time)]
                    # box딕셔너리안에 id없을것이므로 생성
                    track_box[track_id] = []
                # 딕셔너리내 id가 이미 있다면 시간저장
                else:
                    track_history[track_id].append(time.strftime('%Y-%m-%d-%H-%M-%S', p_time))
                    # 만약 id 크기가 일정퍼센트 이상이라면 시간저장
                    if ((width*height)*0.5) <= w*h:
                        track_box[track_id].append(time.strftime('%Y-%m-%d-%H-%M-%S', p_time))
        ...
        if cv2.waitKey(1) != -1:    
            ...
            if track_box:
                ...
                # 저장할 위치
                file_path = f'./json/BOX222_time_{time}.json'
                # 파일 만들어질 시간주기
                time.sleep(1)
                # json파일 저장
                with open(file_path, 'w') as f:
